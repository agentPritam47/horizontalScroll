...
const updateCardPositions = () => {
  ...
  if (state.activeCardIndex !== currentIndex) {
    ...
    // Smoothly scale down previous active card if it exists
    if (state.previousActiveCardIndex !== null && state.projects.has(state.previousActiveCardIndex)) {
      ...
      gsap.to(prevActiveProject, {
        scale: 1,
        duration: config.SCALE_TRANSITION_DURATION,
        ease: config.SCALE_TRANSITION_EASE
      });
    ...
    // Smoothly scale up new active card
  ...
  state.projects.forEach((project, index) => {
    ...
    if (index !== currentIndex) {
      // For non-active cards, just update position
      gsap.set(project, {
        x: `${index * spacing + currentXInVw}vw`,
      });
    } else {
      // For active card, update position but keep scale
      gsap.set(project, {
        x: `${index * spacing + currentXInVw}vw`
      });
    }
